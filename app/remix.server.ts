...
export class Remix extends DurableObject<Env> {
  async fetch(request: Request) {
    ...
    const accessToken = this.getCookie(request, this.env.ACCESS_TOKEN_COOKIE_KEY);
    const refreshToken = this.getCookie(request, this.env.REFRESH_TOKEN_COOKIE_KEY);
    ...
    if (newAccessToken) {
      response.headers.append(
        "Set-Cookie",
        `${this.env.ACCESS_TOKEN_COOKIE_KEY}=${newAccessToken}; HttpOnly; Secure; SameSite=Strict; Max-Age=900; Path=/`
      );
    }
    if (newRefreshToken) {
      response.headers.append(
        "Set-Cookie",
        `${this.env.REFRESH_TOKEN_COOKIE_KEY}=${newRefreshToken}; HttpOnly; Secure; SameSite=Strict; Max-Age=604800; Path=/`
      );
    }
